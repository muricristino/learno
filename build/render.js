...
// ── prop validation ───────────────────────────────────────────────────────
// meta.props is a map of name → type. A trailing ? marks it optional.
// The `svg` type additionally enforces that a diagram carries no colours of
// its own — see the note in assets/learno.css.
...
function validateProps(component, props, report, where) {
  ...
  for (const [name, rawType] of Object.entries(spec)) {
    ...
    if (value === undefined || value === null) {
      if (!optional) report.error(where, `prop "${name}" is required (${type})`);
      continue;
    }
    if (!checkType(value, type)) {
      report.error(where, `prop "${name}" should be ${type}, got ${Array.isArray(value) ? 'array' : typeof value}`);
      continue;
    }
  ...
  for (const name of Object.keys(props)) {
    if (!(name in spec)) report.warn(where, `prop "${name}" is not declared by the "${component.meta.name}" component and will be ignored`);
  }
}
